...
function compareCapabilities() {
  try {
    ...
    Logger.log(`Carregando planilha alvo: ${COMPARE_CONFIG.TARGET_SHEET_NAME}`);
    const targetSheet = spreadsheet.getSheetByName(COMPARE_CONFIG.TARGET_SHEET_NAME);
    
    // Validate sheets exist
    if (!sourceSheet) {
      Logger.log(`ERRO: Planilha "${COMPARE_CONFIG.SOURCE_SHEET_NAME}" não encontrada`);
    ...
    if (!targetSheet) {
      Logger.log(`ERRO: Planilha "${COMPARE_CONFIG.TARGET_SHEET_NAME}" não encontrada`);
    ...
    Logger.log(`Obtendo dados da planilha alvo: ${COMPARE_CONFIG.TARGET_SHEET_NAME}`);
  ...
  sourceData.rows.forEach((sourceRow, sourceRowIndex) => {
    ...
    if (targetMap[key]) {
      ...
      for (let colIndex = 2; colIndex < sourceRow.length; colIndex++) {
        ...
        if (colIndex % COMPARE_CONFIG.LOG_FREQUENCY === 0 || colIndex === 2 || colIndex === sourceRow.length - 1) {
