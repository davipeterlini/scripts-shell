 ...
 * with the CLI-Capabilities sheet to identify any discrepancies in capability status.
 * It includes detailed logging to monitor the execution in the Apps Script logs.
 * 
 * Only compares the following capabilities: StreamingSystem, InstructionChat, ConversationImage,
 * RecognitionFunction, CallingText, and Embedding.
 */
...
const CLI_COMPARE_CONFIG = {
  ...
  // List of capabilities to compare (column names)
  CAPABILITIES_TO_COMPARE: [
    "StreamingSystem",
    "InstructionChat",
    "ConversationImage",
    "RecognitionFunction",
...
function CLI_findStatusDiscrepancies(sourceData, targetData, sourceMap, targetMap, discrepancies) {
  ...
  // Create a map of column indices for capabilities to compare
  const capabilityIndices = {};
  CLI_COMPARE_CONFIG.CAPABILITIES_TO_COMPARE.forEach(capability => {
    const index = sourceData.headers.indexOf(capability);
    if (index !== -1) {
      capabilityIndices[capability] = index;
    }
  });
  
  // Log which capabilities will be compared
  Logger.log("Colunas de capabilities a serem comparadas:");
  Object.keys(capabilityIndices).forEach(capability => {
    const colIndex = capabilityIndices[capability];
    Logger.log(`Coluna ${CLI_columnToLetter(colIndex + 1)} (${colIndex + 1}): ${capability}`);
  });
  ...
  sourceData.rows.forEach((sourceRow, sourceRowIndex) => {
    ...
    if (targetMap[key]) {
      ...
      // Only compare the specified capabilities
      Object.keys(capabilityIndices).forEach(capability => {
        const colIndex = capabilityIndices[capability];
        ...
        if (colIndex % CLI_COMPARE_CONFIG.LOG_FREQUENCY === 0 || colIndex === 2) {
