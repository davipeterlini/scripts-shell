...
function _formatReport(sheet) {
  ...
  // Set background color for data rows (except status cells that are already colored)
  const dataRows = sheet.getRange(4, 1, lastRow - 3, 4); // Columns 1-4
  dataRows.setBackground(COMPARE_CONFIG.COLORS.ERROR_BG);
  
  const locationColumn = sheet.getRange(4, 7, lastRow - 3, 1); // Column 7
  locationColumn.setBackground(COMPARE_CONFIG.COLORS.ERROR_BG);
