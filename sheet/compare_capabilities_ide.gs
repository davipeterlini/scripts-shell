...
function _findStatusDiscrepanciesIDE(sourceData, targetData, sourceMap, targetMap, discrepancies) {
  ...
  COMPARE_CONFIG_IDE.CAPABILITIES_TO_COMPARE.forEach(capability => {
  ...
  Object.keys(capabilityIndices).forEach(capability => {
    ...
    Logger.log(`Column ${_columnToLetterIDE(colIndex + 1)} (${colIndex + 1}): ${capability}`);
  });
  ...
  sourceData.rows.forEach((sourceRow, sourceRowIndex) => {
    ...
    if (targetMap[key]) {
      ...
      Object.keys(capabilityIndices).forEach(capability => {
        ...
        if (colIndex % COMPARE_CONFIG_IDE.LOG_FREQUENCY === 0 || colIndex === 2) {
