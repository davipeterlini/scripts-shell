...
if [[ -f ~/.ssh/id_rsa_bitbucket.pub ]]; then
    echo "An SSH key for Bitbucket already exists."
else
    # Generate a new SSH key for the provided email
    ssh-keygen -t rsa -b 4096 -C "$bitbucket_email" -f ~/.ssh/id_rsa_bitbucket
  ...
  AddKeysToAgent yes
  UseKeychain yes
  IdentityFile ~/.ssh/id_rsa_bitbucket" >> ~/.ssh/config
...
if command -v pbcopy >/dev/null; then
    ...
    xclip -selection clipboard < ~/.ssh/id_rsa_bitbucket.pub
    echo "The SSH public key has been copied to your clipboard."
elif command -v xsel >/dev/null; then
    xsel --clipboard < ~/.ssh/id_rsa_bitbucket.pub
    echo "The SSH public key has been copied to your clipboard."
else
    echo "SSH public key:"
